...
bot = telebot.TeleBot('7285881274:AAGl8K8s8iM-Z68cps-IosfEIAz33aAFkoI')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 500:
                response = "🤬𝘽𝙆𝙇. 𝙏𝙄𝙈𝙀 𝙆𝘼𝙈 𝘿𝘼𝙇 𝙉𝘼 - 500"
            else:
                ...
                response = f"-漫~*'¨¯¨'*·舞~ 🇮🇳ąɬɬąƈƙ ƈơɱ℘Ɩɛɬɛɖ🇮🇳 ~舞*'¨¯¨'*·~漫\n- Target: {target} \nPort: {port} \nTime: {time} \n\n🔻@SPIDER_WOMANs_lover🔻"
        else:
            response = "✅ 𝙇𝙊𝙃𝘼 𝙂𝘼𝙍𝘼𝙈 𝙃 ✅ :- \n\n/bgmi <target> <port> <time>"  # Updated command syntax
    else:
        response = " ❌ 🛑𝘼𝙎𝙎𝙀𝘾𝙀𝙎 𝙇𝙀 𝙇𝙀 𝘽𝙍𝙊🛑 ❌DM - @SPIDER_WOMANs_lover."
...
💥 /admincmd : 𝙎𝙝𝙤𝙬𝙨 𝘼𝙡𝙡 𝙋𝘼𝙋𝘼 𝙂 @SPIDER_WOMANs_lover 𝙆𝙄 𝘾𝙤𝙢𝙢𝙖𝙣𝙙𝙨
...
def welcome_start(message):
    ...
    response = f'''❤️𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙩𝙤 @SPIDER_WOMANs_lover  𝙛𝙖𝙢𝙞𝙡𝙮❤️  \n{user_name}! \n★彡[ᴛᴜᴍ ᴍᴜᴊᴇ ᴄʜᴏʀ ᴋᴀʀ ᴍᴛ ᴊᴀɴᴀ ʙᴀʙᴜ😉]彡★
𒆜🤖🆃🆁🆈 🆃🅾 🆁🆄🅽 🆃🅷🅸🆂 🅲🅾🅼🅼🅰🅽🅳𒆜𓊉 : /help
'''
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, 🎯彡[ʙʀᴏᴛʜᴇʀ ᴏɴʟʏ 1 ᴘʟᴀɴ ɪꜱ ᴘᴏᴡᴇʀꜰᴜʟʟ ᴛʜᴇɴ ᴀɴʏ ᴏᴛʜᴇʀ ᴅᴅᴏꜱ]彡@SPIDER_WOMANs_lover🎯 !!:
